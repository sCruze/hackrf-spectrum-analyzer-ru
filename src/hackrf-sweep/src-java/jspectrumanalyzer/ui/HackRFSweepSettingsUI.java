...
public class HackRFSweepSettingsUI extends JPanel
{
	...
	public HackRFSweepSettingsUI(HackRFSettings hackRFSettings)
	{
		...
		JLabel lblNewLabel = new JLabel("Начальная частота [МГц]");
		...
		JLabel lblFrequencyEndmhz = new JLabel("Конечная частота [МГц]");
		...
		txtHackrfConnected.setText("SDR-приёмник отключен");
		...
		tabbedPane.addTab("Опции SDR-приёмника", tab1);
		tabbedPane.addTab("Опции диаграммы", tab2);
		...
		//tab1
		{
			JLabel lblGain = new JLabel("Усиление [дБ]");
			...
			JLabel lbl_gainValue = new JLabel(hackRFSettings.getGain() + "дБ");
			lbl_gainValue.setForeground(Color.WHITE);
			tab1.add(lbl_gainValue, "cell 0 1,alignx right");

			JLabel lblNewLabel_2 = new JLabel("Усиление LNA [дБ]");
			...
			JLabel lblVgfaGaindb = new JLabel("Усиление VGA [дБ]");
			...
			JLabel lblLNAEnable = new JLabel("Антенный LNA +14дБ");
			...
			JLabel lblFftBinhz = new JLabel("Бин FFT [Гц]");
			...
			hackRFSettings.getGain().addListener((gain) -> lbl_gainValue.setText(String.format(" %dдБ  [LNA: %dдБ  VGA: %dдБ]", 
					gain, hackRFSettings.getGainLNA().getValue(), hackRFSettings.getGainVGA().getValue())));
			

			JLabel lblNumberOfSamples = new JLabel("Число отсчётов");
			...
			JButton btnAbout = new JButton("Переход к сайту");
			...
			JLabel lblAntennaPower = new JLabel("Выходная мощность антенны");
		...
		JLabel lblWaterfallPaletteLength = new JLabel("Длина водопада [дБ]");
		...
		JLabel lblSpurFiltermay = new JLabel("Фильтр спура (может искажать сигнал)");
		...
		lblPeakFall = new JLabel("  Скорость падения [с]");
		...
		lblDecayRate = new JLabel("  Время [с]");
		lblDecayRate.setForeground(Color.WHITE);
		tab2.add(lblDecayRate, "flowx,cell 0 16,growx");
		
		JLabel lblDisplayFrequencyAllocation = new JLabel("Таблица частот");
		...
		lblDebugDisplay = new JLabel("Показ отладки");
	...
	private void bindViewToModel() {
		...
		new MVCController((valueChangedCall) -> btnPause.addActionListener((event) -> valueChangedCall.accept(!hRF.isCapturingPaused().getValue())), 
				isCapt -> btnPause.setText(!isCapt ? "Пауза"  : "Продолжение"), 
...
//				btnPause.setText(isCapturing ? "Pause"  : "Resume");
			...
			{
				txtHackrfConnected.setText("SDR-приёмник "+(hardwareSendingData ? "подключен":"отключен"));
			}
		});;
