...
//			parameterFrequency.setValue(new FrequencyRange(700, 2700));
		...
		labelMessages = new JLabel("Строка отладки");
		...
		uiFrame.setTitle("HTSTS RFMS-Spectrum Analyzer (HTSTS RFMS-SA)");
...
//								waterfallPlot.setStatusMessage(perfWatch.generateStatistics(), 1);
	...
	private void setupChart() {
		...
		chart = ChartFactory.createXYLineChart("Анализатор спектра", "Частота [МГц]", "Мощность [дБ]", chartDataset,
	...
	private void setupChartMouseMarkers() {
		...
		chartPanel.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				...
				signalMarker.setLabel(String.format("%.1fдБ", crosshairRange));
				...
				freqMarker.setLabel(String.format("%.1fМГц", crosshairDomain));
				...
				if (activeTable != null) {
					...
					if (band == null)
						...
						titleFreqBand.setText(String.format("%s - %s МГц  %s", format.format(band.getMHzStartIncl()),
